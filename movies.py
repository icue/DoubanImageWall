...
parser.add_argument('-o', '--offset', type=int, default=13,
                    help='In the first 4 rows, a larger poster will occur once every {offset} cells.'
                    'The following rows repeat this pattern. Set to 0 to disable larger posters.')
...
parser.add_argument('-ht', '--height', type=int, default=750, help='Height of one cell. A value less than 800 is recommended.')
parser.add_argument('-rd', '--random', type=bool, default=False, help='Whether to shuffle the posters')
...
parser.add_argument('-l', '--limit', type=int, default=200,
                    help='Limit of number of movies to process. Default is 200. Raise this when COL x ROW > 200.')
args = parser.parse_args()
_COLUMN_NUM = args.col
_ROW_NUM = args.row
_OFFSET = args.offset
_CELL_WIDTH = args.width
_CELL_HEIGHT = args.height
...
urls = (f'https://movie.douban.com/people/{id}/collect?'
...
for url in urls:
    ...
    movies = soup.find_all('a', {'class': 'nbg'})
    if not movies:
        break
    for a in movies:
        ...
        if len(img_urls) >= _COLUMN_NUM * _ROW_NUM - len(skip_image_index):
            break
    time.sleep(5)
print(f'A total of {len(img_urls)} posters to process')
if args.random:
        ...
        if need_retrieve:
            ...
            try:
                ...
                try:
                    ...
                    print('Still failing! Trying again with larger poster size...')
                    time.sleep(3)
...
result.save(id + '.jpg', optimize=True, quaility=80)
